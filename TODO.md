...
5. `src/basic_mmo_rpg/server/app.py`
   Проработать модель cooldown-а для gathering-а: сейчас cooldown общий для рыбалки и рубки на персонажа, но в будущем может понадобиться отдельная доменная модель действий, инструментов, interrupt-ов и anti-abuse правил.
