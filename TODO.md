...
9. `src/basic_mmo_rpg/server/world.py`, `src/basic_mmo_rpg/client/app.py`
   Проработать respawn игрока: сейчас смерть и возрождение runtime-only, точка респауна фиксируется объектом карты, а восстановление здоровья задано простым правилом 50% от max HP.
