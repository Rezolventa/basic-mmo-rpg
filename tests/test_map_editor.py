...
def test_create_empty_map_from_template_writes_valid_map(tmp_path: Path) -> None:
    ...
    assert entity_positions["object-spinning-wheel"] == [64, 32]
    assert entity_positions["creature-barbara"] == [96, 32]
    assert entity_positions["object-player-respawn"] == [128, 32]
    assert entity_positions["creature-boar"] == [0, 64]
    assert entity_positions["lootable-training-dummy"] == [32, 64]
